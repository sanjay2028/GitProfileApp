...
const RepoRow = ({item, onShowModal}) => {  
  ...
  return (
    <tr key={id}>
      ...
      <td>{name}</td> 
      <td>{language}</td> 
      <td>{commits.length} Commit(s)</td> 
      <td><button onClick={(event) => onShowModal(event) } id={_id} className="btn btn-primary">Show Commits</button></td>
    </tr>
  )
}
        ...
        <main role="main" className="col-md-9 ml-sm-auto col-lg-10 pt-3 px-4">
          ...
          <div className="table-responsive">
            <table className="table table-striped table-sm">
              <thead>
                <tr><th>#</th><th>Name</th><th>Language</th><th>Commits</th><th>Action</th></tr>
              </thead>
              <tbody>{output}</tbody>
            </table>
          </div>
